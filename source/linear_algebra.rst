...
.. math::
    ...
    \begin{flalign}
    & \text{Let } A \text{ be a } m\times n \text{ matrix, } A=(a_{ij})\in M_{mn}(\Bbb F) \\
    & \text{Let } B \text{ be a } n\times p \text{ matrix, } B=(b_{ij})\in M_{np}(\Bbb F) \\
    \end{flalign}
